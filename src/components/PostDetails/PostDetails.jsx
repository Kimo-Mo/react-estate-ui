...
const PostDetails = () => {
  const { listData, setMenuActive , userData } = useContext(PostContext);
  const { ItemId } = useParams();
  const post =
    listData.length > 0 && listData.find((p) => p.id == Number(ItemId));
  const [mainImgSrc, setMainImgSrc] = useState(null);
  useEffect(() => {
    listData.length > 0 && setMainImgSrc(post.img);
  }, [listData, post]);
  const smallImgs = [
          ...
          <div className="head d-flex justify-content-between flex-column flex-md-row gap-5">
            <div className="left d-flex justify-content-between flex-column">
              ...
              <p className="location">
                <img src="/pin.png" alt="location image" className="iconImg" />
                {post.address}
              </p>
